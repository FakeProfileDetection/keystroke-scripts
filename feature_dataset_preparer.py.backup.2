...
def analyze_platform_leakage(df: pd.DataFrame):
    """Enhanced platform leakage analysis with better visualization"""
    import matplotlib.pyplot as plt
    ...
    if acc > 0.8:
        ...
        print("⚠️ MODERATE LEAKAGE RISK - Some platform-specific patterns detected")
    else:
    ...
    for name, score in top_features:
        risk_level = "🚨" if score > 0.1 else "⚠️" if score > 0.05 else "✅"
        print(f"  {risk_level} {name:35} → {score:.4f}")
    ...
    sns.barplot(
        ...
        y=[name[:30] + '...' if len(name) > 30 else name for name, _ in top_features],
        palette=colors
    )
    ...
    # Save plot
    plt.savefig("platform_leakage_analysis.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def run_experiments(df: pd.DataFrame):
    ...
    print("\n🚀 Starting comprehensive ML experiments...")
    ...
    for exp_idx, (train_platforms, test_platform, experiment_name) in enumerate(experiments, 1):
        ...
        if config.get("show_class_distributions", False):
            ...
            plt.tight_layout()
            plt.savefig(f"class_distribution_{experiment_name}.png", dpi=300, bbox_inches='tight')
            plt.show()
            plt.close()

        # Run all models with different scalers
        ...
        for scaler in scalers_to_test:
            ...
            try:
                ...
                print(f"❌ Random Forest failed with {scaler.name}: {e}")
