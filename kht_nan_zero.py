...
from lori_keystroke_features import read_compact_format
...
def check_zeros_and_nans(data_dict):
    total_count = 0

    for _, vals in data_dict.items():
        # Ensure vals is a numpy array or a list
        if isinstance(vals, (list, np.ndarray)):
            vals = np.array(vals)  # Convert to numpy array if it's not already

            nan_count = np.isnan(vals).sum()  # Count NaNs
            zero_count = (vals == 0).sum()  # Count zeros
            total_count += nan_count + zero_count
        else:
            print(f"Warning: Data for {_} is not a list or numpy array. Skipping.")
    ...
    for i in track(all_ids()):
        for j in (1, 2, 3):
            ...
            if df.empty:
                ...
                continue
            bads = check_zeros_and_nans(create_kht_data_from_df(df))
            print(f"User: {i}, platform: {map_platform_id_to_initial(j)}, bads: {bads}")
    for i in track(all_ids()):
        for j in (1, 2, 3):
            df = get_user_by_platform(i, j)
            if df.empty:
                ...
                continue
            for f in (1,2,3,4):
                bads = check_zeros_and_nans(create_kit_data_from_df(df,j))
                print(f"User: {i}, platform: {map_platform_id_to_initial(j)}, feature_type: {f}, bads: {bads}")
