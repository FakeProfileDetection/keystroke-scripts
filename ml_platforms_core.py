...
class ModelTrainer:
    ...
    def train_random_forest(self, X_train: np.ndarray, X_test: np.ndarray, 
        ...
        if self.config.draw_feature_importance:
            visualizer.plot_feature_importance(model, "RandomForest", experiment_name, 
    ...
    def train_xgboost(self, X_train: np.ndarray, X_test: np.ndarray, 
        ...
        # Feature importance plot
        if self.config.draw_feature_importance:
            visualizer.plot_feature_importance(model, "XGBoost", experiment_name, 
                                             [f"feature_{i}" for i in range(X_train.shape[1])])
    ...
    def train_catboost(self, X_train: np.ndarray, X_test: np.ndarray, 
        ...
        # Feature importance plot
        if self.config.draw_feature_importance:
            visualizer.plot_feature_importance(model, "CatBoost", experiment_name, 
                                             [f"feature_{i}" for i in range(X_train.shape[1])])
