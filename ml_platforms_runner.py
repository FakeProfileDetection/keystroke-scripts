...
class MLExperimentRunner:
    ...
    def __init__(
        self, config: ExperimentConfig, max_workers: int = None, use_gpu: bool = True
    ):
        self.config = config
        ...
        self.trainer = ModelTrainer(
            config, self.output_dir, self.timestamp, use_gpu=self.use_gpu
        )
        ...
        if config.debug_mode:
            ...
            self.experiments = [([1, 2], 3, "FI_vs_T"), ([2], 1, "I_vs_F")]
    ...
    def run_experiments(self, df: pl.DataFrame):
        ...
        for exp_idx, (train_platforms, test_platform, exp_name) in enumerate(
            self.experiments, 1
        ):
            print(f"\n{'='*60}")
            print(f"🎯 Experiment {exp_idx}/{len(self.experiments)}: {exp_name}")
            print(
                f"Training on platforms: {train_platforms}, Testing on: {test_platform}"
            )
            print(f"{'='*60}")
            ...
            feature_cols = [
                ...
                if col not in {"user_id", "platform_id", "session_id"}
            ]
            ...
            print(
                f"📊 Train classes: {len(train_class_counts)}, Test classes: {len(test_class_counts)}"
            )
            ...
            if self.config.show_class_distributions:
                ...
                train_class_counts.head(20).plot(
                    kind="bar", ax=ax1, title=f"Train Classes - {exp_name}"
                )
                ax1.set_xlabel("User ID")
                ax1.set_ylabel("Sample Count")

                test_class_counts.head(20).plot(
                    kind="bar", ax=ax2, title=f"Test Classes - {exp_name}"
                )
                ...
                plt.tight_layout()
                plt.savefig(
                    self.output_dir / f"class_distribution_{exp_name}.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close()
            ...
            for seed in self.config.random_seeds:
                ...
                for model_name, train_func in models_to_run:
                    ...
                    try:
                        ...
                        model = (
                            self.trainer.label_encoder.classes_
                        )  # Get the trained model's label classes
                        ...
                        for k in range(1, 6):
                            detailed_record = {
                                ...
                                "k_value": k,
                                "train_top_k_accuracy": result.train_metrics.get(
                                    f"train_top_{k}_accuracy", 0
                                ),
                                "test_top_k_accuracy": result.test_metrics.get(
                                    f"test_top_{k}_accuracy", 0
                                ),
                        ...
                        print(
                            f"✅ {model_name}: Top-1 = {result.test_metrics.get('test_top_1_accuracy', 0):.4f}, "
    ...
    def generate_comprehensive_report(self):
        ...
        detailed_csv_path = (
            self.output_dir / f"detailed_topk_results_{self.timestamp}.csv"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clean ML experiments for keystroke biometrics"
    )
    parser.add_argument("-d", "--dataset", required=True, help="Path to dataset CSV")
    parser.add_argument(
        "-e", "--early-stop", action="store_true", help="Use early stopping"
    )
    parser.add_argument(
        "-s", "--seeds", type=int, default=1, help="Number of random seeds"
    )
    parser.add_argument(
        "-o", "--output-affix", default="", help="Output directory suffix"
    )
    parser.add_argument(
        "--show-class-dist", action="store_true", help="Show class distribution plots"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-workers", type=int, default=16, help="Max CPU workers (default: 16)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug mode (minimal hyperparameters)",
    )
    ...
    runner = MLExperimentRunner(
        config, max_workers=args.max_workers, use_gpu=not args.no_gpu
    )
    df = runner.load_data()
    ...
    print(
        f"🌐 Open the HTML report to view results: {runner.output_dir}/user_identification_report_{runner.timestamp}.html"
    )
